document.addEventListener("DOMContentLoaded", () => {
    ...
    /* Añadimos el contenedor que tiene los botones justo después del encabezado con '.insertAdjacentElement' seguido de dónde se va a añadir (afterend, que es después del final) y el qué se quiere añadir */
    ...
    function filtrarProductosPorMarca(marcaSeleccionada) {
        ...
        productos.forEach(producto => {
            ...
            // Hacemos un condicional para mostrar u ocultar el producto según si la marca coincide con la seleccionada o no
            if (marcaProducto && marcaProducto.toLowerCase() === marcaSeleccionada.toLowerCase()) {
                producto.style.display = 'block'; // Mostramos el producto si la marca coincide
            } else {
                producto.style.display = 'none'; // Ocultamos el producto si la marca no coincide
            }
        });
    }

    /* Por último asignamos las marcas a los productos desde JS */
    document.querySelectorAll('.productos_article').forEach((producto) => {
        if (producto.querySelector('.titulito_producto')) {
            ...
            // Ahora hacemos condicionales para indicar el coche que va a aparecer según el botón que se clickee
